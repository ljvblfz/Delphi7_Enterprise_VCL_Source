...
{$IFDEF MSWINDOWS}
  Windows,
{$ENDIF}
  HTTPApp, Classes, SyncObjs,
  SvrConst, QControls, Qt;

const
  QEventType_LogMessage = QEventType(Integer(QEventType_ClxUser) + $01);
...
type
 ...
 protected
  ...
  TLogControl = class(TWidgetControl)
  private
    FServerLog: TCustomServerLog;
  protected
    function EventFilter(Sender: QObjectH; Event: QEventH): Boolean; override;
  public
    constructor Create(AServerLog: TCustomServerLog);
  end;

implementation

uses SysUtils, QForms, SvrHTTPIndy, HTTPParse, IdHTTPServer;
...
begin
  inherited Create;
  FWidgetControl := TLogControl.Create(Self);
...
begin
  ...
  try
    ...
    QApplication_postEvent(FWidgetControl.Handle, QCustomEvent_create(QEventType_LogMessage, nil));
...
{ TLogControl }
constructor TLogControl.Create(AServerLog: TCustomServerLog);
begin
  inherited Create(nil);
  FServerLog := AServerLog;
end;

function TLogControl.EventFilter(Sender: QObjectH;
...
begin
  ...
  case EventType of
    QEventType_LogMessage:  FServerLog.WriteLog;
  end;
