...
uses 
  SysUtils, OrbPas45, Classes, VarUtils, Variants;

type
  _Object = System.IUnknown;
  Any = System.Variant;
  TypeCode = ORBPAS45.ITypeCode;
  TCommandLine = ORBPAS45.TArgv;
  StructMemberSeq = ORBPAS45.TStructMembers;
  UnionMemberSeq = ORBPAS45.TUnionMembers;
  RegistrationScope = ORBPAS45._RegistrationScope;


  PExceptionProxy = ORBPAS45.PUserExceptionProxy;
  PExceptionDescription = ORBPAS45.PExceptionDescription;
  ...
  protected
    ...
    procedure _Copy(const InBuf: ORBPAS45.MarshalInBufferProxy);
  public
    constructor Create;
  ...
  protected
    FProxy: ORBPAS45.ObjectProxy;
    function Proxy: IUnknown;
    constructor Create(const Proxy: ORBPAS45.ObjectProxy); overload;
...
type
  ...
  private
    FProxy : ORBPAS45.MarshalInBufferProxy;
    function Proxy: IUnknown;
  public
    procedure ReadBoolean(out Value: Boolean);
    procedure ReadChar(out Value: Char);
    ...
    constructor Create(Proxy: ORBPAS45.MarshalInBufferProxy);
    destructor Destroy; override;
  end;

  TOutputStream = class(TInterfacedObject, OutputStream, ProxyUser)
  private
    FProxy : ORBPAS45.MarshalOutBufferProxy;
    function Proxy: IUnknown;
  public
    function CreateInputStream: InputStream;
    procedure WriteBoolean(Value: Boolean);
    ...
    constructor Create(Proxy: ORBPAS45.MarshalOutBufferProxy);
    destructor Destroy; override;
  end;
...
procedure PBindOptionsToBindOptions(Source: ORBPAS45.PBindOptions; var Dest: BindOptions);
begin
...
procedure BindOptionsToPBindOptions(const Source: BindOptions; var Dest: ORBPAS45.BindOptions);
begin
...
constructor TCORBAObject.Create(const Proxy: ORBPAS45.ObjectProxy);
begin
  FProxy := Proxy;
...
begin
  with Obj as ProxyUser do
     Create(Proxy as ORBPAS45.ObjectProxy);
end;

constructor TCORBAObject.CreateSkeleton(const InstanceName, InterfaceName, RepositoryId: string);
var Proxy : ORBPAS45.ObjectProxy;
begin
  ORBPAS45.CreateSkeletonProxy(PChar(Pointer(InterfaceName)), Self,
...
var
  Proxy: ORBPAS45.MarshalOutBufferProxy;
begin
...
begin
  FProxy.invoke((Output as ProxyUser).Proxy as ORBPAS45.MarshalOutBufferProxy, Proxy);
...
begin
  Result := FProxy.is_equivalent((OtherObject as ProxyUser).Proxy as ORBPAS45.ObjectProxy);
end;
...
constructor TInputStream.Create(Proxy: ORBPAS45.MarshalInBufferProxy);
begin
  FProxy := Proxy;
end;
...
var
  Proxy, tempProxy: ORBPAS45.ObjectProxy;
begin
...
constructor TOutputStream.Create(Proxy: ORBPAS45.MarshalOutBufferProxy);
begin
  FProxy := Proxy;
end;
...
begin
  ...
  FProxy.put_object((Value as ProxyUser).Proxy as ORBPAS45.ObjectProxy);
end;
...
begin
  inherited Create(ClassName);
  FProxy := ORBPAS45.CreateUserException(_Copy, Throw)
end;

procedure UserException._Copy(const InBuf: ORBPAS45.MarshalInBufferProxy);
begin
...
begin
  ...
  POA.ActivateObject(PChar(Pointer(ObjectID)),
    ((Obj as ProxyUser).Proxy as ORBPAS45.ObjectProxy).CorbaObject);
end;
...
begin
  BOA.ObjIsReady( ((Obj as ProxyUser).Proxy as ORBPAS45.ObjectProxy).CorbaObject);
end;
...
var
  P: PChar;
begin
  P := FProxy.object_to_string((Obj as ProxyUser).Proxy as ORBPAS45.ObjectProxy);
  Result := P;
end;
...
begin
  ...
  FProxy.Put_Any(CorbaAny, tc,
    (Output as ProxyUser).Proxy as ORBPAS45.MarshalOutBufferProxy);
...
//            (Output as ProxyUser).Proxy as ORBPAS45.MarshalOutBufferProxy, Result);
end;
...
begin
   ...
   result := ORBPAS45.RegisterUserException( PChar(Pointer (Name)), PChar(Pointer (RepositoryID)), Factory);
end;

procedure UnRegisterUserException(Description: PExceptionDescription);
begin
   ORBPAS45.UnRegisterUserException(Description);
end;
