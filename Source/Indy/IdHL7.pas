...
{
 Version History:
   13/04/2002   Grahame Grieve      Fixed SingleThreadMode timeout issues + WaitForConnection
...
type
  TMessageArriveEvent = procedure(ASender: TObject; AMsg: String) of object;
  TMessageReceiveEvent = procedure(ASender: TObject; AMsg: String; var VHandled: Boolean; var VReply: String) of object;
  ...
  Protected
    ...
    function HandleMessage(const AMsg: String; var VReply: String): Boolean;
  Public
...
begin
  ...
  until LFinished or (GetTickDiff(LStartTime,GetTickCount) > LWaitStop);
  ...
  try
    ...
    repeat
      ...
      if (LStart > 0) and (LEnd > 0) then
        ...
        if HandleMessage(LMsg, LReply) then
          begin
          if LReply <> '' then
            begin
...
function TIdHL7.HandleMessage(const AMsg: String; var VReply: String): Boolean;
var 
  ...
  try
      ...
      cmSynchronous, cmSingleThread:
        ...
        if IsListener then
          ...
          if FCommunicationMode = cmSynchronous then
            begin
            Result := False;
            FOnReceiveMessage(self, AMsg, Result, VReply)
            end
          else
            begin
    ...
    Exception do
      if Assigned(FOnReceiveError) then
        begin
        FOnReceiveError(self, AMsg, e, VReply, Result)
        end
    else
      begin
      Result := False;
