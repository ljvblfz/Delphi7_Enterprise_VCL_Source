...
begin
  LVoidStream := TMemoryStream.Create;
  try
    IOHandler := TIdIOHandlerStream.Create(nil);
    try
      TIdIOHandlerStream(IOHandler).ReadStream := AStream;
      TIdIOHandlerStream(IOHandler).WriteStream := LVoidStream;
      ReceiveHeader(AMsg);
      if not AHeaderOnly then
      begin
        ReceiveBody(AMsg);
