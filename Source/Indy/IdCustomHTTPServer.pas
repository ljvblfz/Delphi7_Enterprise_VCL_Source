          ...
          LRequestInfo := TIdHTTPRequestInfo.Create; try
            ...
            if Length(LRequestInfo.QueryParams) > 0 then begin
              if Length(LRequestInfo.UnparsedParams) = 0 then begin
                ...
                LRequestInfo.FUnparsedParams := LRequestInfo.UnparsedParams + EOL
            ...
            LResponseInfo := TIdHTTPResponseInfo.Create(AThread.Connection); try
              LResponseInfo.CloseConnection := not FKeepAlive;
              // Session management
              ...
              if Length(Trim(ServerSoftware)) > 0  then begin
                LResponseInfo.FServerSoftware := ServerSoftware;
              end;
              try
...
begin
  ...
  begin
    ...
    begin
      Values['Last-Modified'] := DateTimeToInternetStr(FLastModified); {do not localize}
    end;
