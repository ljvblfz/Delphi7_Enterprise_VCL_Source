...
{$ENDIF}
  ...
  public
    ...
    function LookupHostAddr(const hn: string): TSocketHost;
    function LookupPort(const sn: string; pn: pchar = nil): word;
    function LookupProtocol(const pn: string): TSocketProtocol;
...
function TIpSocket.LookupPort(const sn: string; pn: pchar): word;
var
  se: PServent;
begin
  Result := 0;
...
function TIpSocket.LookupProtocol(const pn: string): TSocketProtocol;
var
  pe: PProtoent;
begin
  Result := 0;
  pe := getprotobyname(pchar(pn));
  if pe <> nil then
    Result := pe^.p_proto;
end;

function TIpSocket.LocalDomainName: string;
var
{$IFDEF MSWINDOWS}
  dname: pchar;
{$ENDIF}
...
begin
  Result := '';
{$IFDEF MSWINDOWS}
  dname := strpos(pchar(LookupHostName(LocalHostAddr)), '.');
  if dname <> nil then
    Result := dname + 1;
{$ENDIF}
{$IFDEF LINUX}
