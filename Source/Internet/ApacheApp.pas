...
begin
  ...
  P := PChar(S);
  if Length(S) + 1 > sizeof(ModuleName) then
    // Exclude first part of path if path is very long
    P := P + (Length(S)+ 1) - sizeof(ModuleName);
  StrLCopy(ModuleName, P, sizeof(ModuleName));
