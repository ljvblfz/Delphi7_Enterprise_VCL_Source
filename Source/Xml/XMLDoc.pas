...
begin
  ...
  begin
    if ChangeType = ncAddAttribute then
      DOMElement.setAttribute(AttrName, Value) else
...
{ This method uses an explicit namespaceURI. }
  ...
  DoNodeChange(ChangeType, True);
  if ChangeType = ncAddAttribute then
    DOMElement.setAttributeNS(NamespaceURI, NodeName, Value) else
...
begin
  Result := Supports(Node, ItemInterface)
    and (ExtractLocalName(Node.NodeName) = ItemTag);
end;
