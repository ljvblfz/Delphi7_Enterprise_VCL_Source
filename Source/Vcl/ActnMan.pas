...
begin
  ...
  begin
    ...
    try
      ...
      if Assigned(TActionClientItem(Source).FContextItems) then
        ContextItems.Assign(TActionClientItem(Source).FContextItems);
...
begin
  ...
  begin
    ...
    AC := ActionClient;
    try
// TODO: This causes Reset to fail and actually delete all of items instead
//!!      ActionClient := nil;
...
var
  NewWidth: Integer;
begin
  // TODO: Need to reexamine Autosize/Floating issues
...
var
  ARect: TRect;
begin
  if Assigned(ActionBar) and Assigned(ActionClient) and
     (not ActionBar.ActionClient.HasBackground or
     (ActionBar.ActionClient.HasBackground and
     (ActionBar.ActionClient.BackgroundLayout in [blLeftBanner, blRightBanner]))) and
     ThemeServices.ThemesEnabled and (Canvas.Brush.Color = ActionBar.ColorMap.Color) then
  begin
    ARect := BoundsRect;
    ThemeServices.DrawParentBackground(ActionBar.Handle, Canvas.Handle, nil, False, @ARect);
